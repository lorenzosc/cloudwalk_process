...
if __name__ == "__main__": 
    ...
    query = """
            ...
            FROM clients
            WHERE DATE '2024-01-25' - DATE(created_at) <= 7;
            """
    cur.execute(query)
    ...
    query = """
            ...
            FROM loans
            WHERE DATE '2024-01-25' - DATE(paid_at) <= 7;
            """
    cur.execute(query)
    ...
    query = """
        ...
        FROM loans
        WHERE 
            DATE '2024-01-25' - DATE(due_at) <= 7
            AND DATE '2024-01-25' - DATE(due_at) >= 0
            AND status = %s;
        """
    ...
    query = """
            ...
            FROM loans
            WHERE DATE '2024-01-25' - DATE(created_at) > 90
            GROUP BY month_year
...
that, we lost R${-1*money_loss:.2f}, and the total still due from those clients is R${still_due:.2f}.
